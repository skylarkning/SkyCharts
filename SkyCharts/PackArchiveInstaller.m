...
- (id)initWithURL:(NSURL *)url chartRoot:(NSString *)root jobID:(NSString *)jobID expectedBytes:(long long)bytes expectedFiles:(NSUInteger)files progressTarget:(id)target selector:(SEL)selector {
    if((self=[super init])){_archiveURL=[url retain];_chartRoot=[root copy];_jobID=[jobID copy];_expectedBytes=bytes;_expectedFiles=files;_progressTarget=target;_progressSelector=selector;_headerBuffer=[[NSMutableData alloc]initWithCapacity:512];_extractedPaths=[[NSMutableSet alloc]init];_outputFD=-1;_archiveOK=YES;}return self;
}
...
- (void)closeOutput { if(_outputFD>=0){close(_outputFD);_outputFD=-1;}[_currentRelativePath release];_currentRelativePath=nil; }
- (void)finishOutput { if(_outputFD>=0){close(_outputFD);_outputFD=-1;}if(_currentRelativePath){[_extractedPaths addObject:_currentRelativePath];_extractedFiles++;[_currentRelativePath release];_currentRelativePath=nil;} }
- (BOOL)openHeader:(const unsigned char *)header {
    ...
    _fileRemaining=SkyChartsTarOctal(header+124,12);_paddingRemaining=(512-(_fileRemaining%512))%512;NSString *target=[_incomingPath stringByAppendingPathComponent:name],*directory=[target stringByDeletingLastPathComponent];NSFileManager *fm=[NSFileManager defaultManager];if(![_lastDirectory isEqualToString:directory]){if(![fm createDirectoryAtPath:directory withIntermediateDirectories:YES attributes:nil error:NULL]){[self fail:@"Could not create an extracted chart directory"];return NO;}[_lastDirectory release];_lastDirectory=[directory copy];}_outputFD=open([target fileSystemRepresentation],O_WRONLY|O_CREAT|O_TRUNC,0600);if(_outputFD<0){[self fail:@"Could not open an extracted chart file"];return NO;}_currentRelativePath=[name copy];if(!_fileRemaining)[self finishOutput];return YES;
}
- (void)consumeData:(NSData *)data {
    ...
    while(offset<length&&_archiveOK&&!_sawArchiveEnd){
        if(_fileRemaining){NSUInteger amount=(NSUInteger)MIN((unsigned long long)(length-offset),_fileRemaining),written=0;while(written<amount){ssize_t result=write(_outputFD,bytes+offset+written,amount-written);if(result<0&&errno==EINTR)continue;if(result<=0){[self fail:@"Could not write an extracted chart file"];break;}written+=(NSUInteger)result;}if(!_archiveOK)break;offset+=amount;_fileRemaining-=amount;if(!_fileRemaining)[self finishOutput];continue;}
...
- (void)dealloc { [_connection cancel];[_connection release];[self closeOutput];[_archiveURL release];[_chartRoot release];[_jobID release];[_incomingPath release];[_errorText release];[_lastDirectory release];[_headerBuffer release];[_extractedPaths release];[super dealloc]; }
@end
