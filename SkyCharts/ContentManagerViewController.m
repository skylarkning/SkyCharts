...
- (void)reloadContent {
    ...
    for(NSString *continent in [self sortedKeys:tree]){
        ...
        NSMutableArray *targets=[NSMutableArray array];NSMutableSet *files=[NSMutableSet set];[self addTargetsFromNodes:countryNodes toArray:targets];[self addFilesFromNodes:countryNodes toSet:files];NSString *countryRegionCount=countryNodes.count==1?@"country/region":@"countries/regions";[roots addObject:[self node:continent subtitle:[NSString stringWithFormat:@"%lu %@",(unsigned long)countryNodes.count,countryRegionCount] key:continent level:0 children:countryNodes kind:@"continent" targets:targets files:files sizes:fileSizes]];
    }
...
- (NSString *)tableView:(UITableView *)table titleForDeleteConfirmationButtonForRowAtIndexPath:(NSIndexPath *)path { NSString *kind=[[[self.rows objectAtIndex:path.row]objectForKey:@"kind"]capitalizedString];if([kind isEqualToString:@"Country"])kind=@"Country/Region";return [NSString stringWithFormat:@"Delete %@",kind]; }
...
- (void)deleteTargetsBackground:(NSArray *)targets {
    ...
    NSString *mapRoot=SkyChartsAirportMapRoot();for(NSString *ident in deletedIdents)[fm removeItemAtPath:[mapRoot stringByAppendingPathComponent:[ident stringByAppendingString:@".json"]]error:NULL];
