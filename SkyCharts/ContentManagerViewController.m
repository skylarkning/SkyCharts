...
- (void)addAssetsFromAirport:(NSDictionary *)airport toSet:(NSMutableSet *)assets { for(NSDictionary *category in [airport objectForKey:@"categories"])for(NSDictionary *chart in [category objectForKey:@"charts"])for(NSDictionary *page in [chart objectForKey:@"pages"]){NSString *path=[page objectForKey:@"light"];if(path.length)[assets addObject:path];} }
