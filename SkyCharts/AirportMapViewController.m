...
- (void)drawRect:(CGRect)rect {
    ...
    for(NSDictionary *feature in self.taxiwayFeatures){UIBezierPath *path=[self pathForFeature:feature];if(!path)continue;path.lineCapStyle=kCGLineCapRound;path.lineJoinStyle=kCGLineJoinRound;[[UIColor colorWithRed:1 green:.79 blue:.03 alpha:1]setStroke];path.lineWidth=.78/MAX(.01,self.labelScale);[path stroke];}
    for(NSDictionary *runway in self.runwayRecords)[self drawRunway:runway context:context];
    NSMutableArray *occupied=[NSMutableArray array];UIFont *runwayFont=[self labelFontWithScreenSize:15];for(NSDictionary *runway in self.runwayRecords){CGPoint start=[[runway objectForKey:@"start"]CGPointValue],end=[[runway objectForKey:@"end"]CGPointValue];CGFloat dx=end.x-start.x,dy=end.y-start.y,length=hypot(dx,dy),step=85/MAX(.01,self.labelScale);NSInteger samples=MAX(2,(NSInteger)ceil(length/step));for(NSInteger sample=0;sample<=samples;sample++){CGFloat fraction=(CGFloat)sample/samples,half=9/MAX(.01,self.labelScale);CGPoint point=CGPointMake(start.x+dx*fraction,start.y+dy*fraction);[occupied addObject:[NSValue valueWithCGRect:CGRectMake(point.x-half,point.y-half,half*2,half*2)]];}NSString *reference=[runway objectForKey:@"ref"];NSArray *ends=[reference componentsSeparatedByString:@"/"];NSString *first=ends.count?[ends objectAtIndex:0]:reference,*last=ends.count>1?[ends lastObject]:reference;for(NSDictionary *marker in [NSArray arrayWithObjects:[NSDictionary dictionaryWithObjectsAndKeys:first,@"text",[runway objectForKey:@"start"],@"point",nil],[NSDictionary dictionaryWithObjectsAndKeys:last,@"text",[runway objectForKey:@"end"],@"point",nil],nil]){NSString *text=[marker objectForKey:@"text"];CGPoint point=[[marker objectForKey:@"point"]CGPointValue];CGRect box=SkyLabelRect(text,point,runwayFont,self.labelScale);SkyDrawLabel(text,point,runwayFont,[UIColor whiteColor],[UIColor colorWithWhite:.02 alpha:.94],[UIColor colorWithWhite:.76 alpha:1],self.labelScale);[occupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-4/MAX(.01,self.labelScale),-4/MAX(.01,self.labelScale))]];}}
    UIFont *taxiFont=[self labelFontWithScreenSize:10];NSMutableDictionary *taxiCandidates=[NSMutableDictionary dictionary];for(NSDictionary *feature in self.taxiwayFeatures){NSString *reference=[[[feature objectForKey:@"ref"]?:@"" stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]uppercaseString];if(!reference.length||reference.length>6)continue;NSMutableArray *candidates=[taxiCandidates objectForKey:reference];if(!candidates){candidates=[NSMutableArray array];[taxiCandidates setObject:candidates forKey:reference];}[candidates addObjectsFromArray:[self markerPointsForFeature:feature]];}for(NSString *reference in [[taxiCandidates allKeys]sortedArrayUsingSelector:@selector(compare:)]){NSMutableArray *accepted=[NSMutableArray array];for(NSValue *value in [taxiCandidates objectForKey:reference]){CGPoint point=[value CGPointValue];BOOL tooClose=NO;for(NSValue *priorValue in accepted){CGPoint prior=[priorValue CGPointValue];if(hypot(point.x-prior.x,point.y-prior.y)<82/MAX(.01,self.labelScale)){tooClose=YES;break;}}if(tooClose)continue;CGRect box=SkyLabelRect(reference,point,taxiFont,self.labelScale);if(!SkyLabelFits(box,occupied,4,self.labelScale))continue;SkyDrawLabel(reference,point,taxiFont,[UIColor colorWithRed:1 green:.86 blue:.08 alpha:1],[UIColor colorWithWhite:.03 alpha:.90],[UIColor colorWithWhite:.22 alpha:1],self.labelScale);[accepted addObject:value];[occupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-4/MAX(.01,self.labelScale),-4/MAX(.01,self.labelScale))]];}}
    ...
    NSMutableArray *standOccupied=[NSMutableArray arrayWithArray:occupied];NSMutableSet *drawnStands=[NSMutableSet set];UIFont *standFont=[UIFont boldSystemFontOfSize:8.5/standScale];for(NSInteger pass=0;pass<2;pass++)for(NSDictionary *feature in self.standFeatures){NSString *kind=[feature objectForKey:@"kind"];if((pass==0&&![kind isEqual:@"gate"])||(pass==1&&![kind isEqual:@"parking_position"]))continue;NSString *reference=[[feature objectForKey:@"ref"]?:[feature objectForKey:@"name"] description];NSArray *points=[feature objectForKey:@"points"];if(!reference.length||!points.count)continue;CGPoint anchor=[self mapPoint:[points lastObject]],point=CGPointMake(anchor.x,anchor.y-7/standScale);CGRect box=SkyLabelRect(reference,point,standFont,standScale);if(!SkyLabelFits(box,standOccupied,5,standScale))continue;CGContextSetStrokeColorWithColor(context,[UIColor colorWithWhite:.72 alpha:.7].CGColor);CGContextSetLineWidth(context,.6/standScale);CGContextMoveToPoint(context,anchor.x,anchor.y);CGContextAddLineToPoint(context,point.x,point.y);CGContextStrokePath(context);SkyDrawLabel(reference,point,standFont,[UIColor colorWithWhite:.92 alpha:1],[UIColor colorWithWhite:.06 alpha:.78],[UIColor colorWithWhite:.55 alpha:.8],standScale);[standOccupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-5/standScale,-5/standScale)]];[drawnStands addObject:[NSValue valueWithNonretainedObject:feature]];}
    CGFloat deepScale=MAX(.01,self.labelScale*4.2);UIFont *deepFont=[UIFont boldSystemFontOfSize:8/deepScale];NSMutableArray *deepOccupied=[NSMutableArray arrayWithArray:standOccupied];for(NSInteger pass=0;pass<2;pass++)for(NSDictionary *feature in self.standFeatures){NSValue *identity=[NSValue valueWithNonretainedObject:feature];if([drawnStands containsObject:identity])continue;NSString *kind=[feature objectForKey:@"kind"];if((pass==0&&![kind isEqual:@"gate"])||(pass==1&&![kind isEqual:@"parking_position"]))continue;NSString *reference=[[feature objectForKey:@"ref"]?:[feature objectForKey:@"name"] description];NSArray *points=[feature objectForKey:@"points"];if(!reference.length||!points.count)continue;CGPoint anchor=[self mapPoint:[points lastObject]],point=CGPointMake(anchor.x,anchor.y-6/deepScale);CGRect box=SkyLabelRect(reference,point,deepFont,deepScale);if(!SkyLabelFits(box,deepOccupied,4,deepScale))continue;CGContextSetStrokeColorWithColor(context,[UIColor colorWithWhite:.65 alpha:.55].CGColor);CGContextSetLineWidth(context,.5/deepScale);CGContextMoveToPoint(context,anchor.x,anchor.y);CGContextAddLineToPoint(context,point.x,point.y);CGContextStrokePath(context);SkyDrawLabel(reference,point,deepFont,[UIColor colorWithWhite:.88 alpha:.92],[UIColor colorWithWhite:.07 alpha:.72],[UIColor colorWithWhite:.46 alpha:.7],deepScale);[deepOccupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-4/deepScale,-4/deepScale)]];}
