...
- (void)configureForDisplayScale:(CGFloat)displayScale;
...
- (void)configureForDisplayScale:(CGFloat)displayScale { if(displayScale>0&&fabs(displayScale-self.labelScale)>.01){self.labelScale=displayScale;[self updateTaxiCenterlineForDisplayScale:displayScale];[self setNeedsDisplay];} }
- (NSDictionary *)featureAtPoint:(CGPoint)point tolerance:(CGFloat)tolerance {
...
static NSString *SkyTerminalLabel(NSDictionary *feature) {
    ...
    if(label.length)return [label uppercaseString];
    if(reference.length){NSString *value=[[reference stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]uppercaseString];if(value.length==1&&[[NSCharacterSet alphanumericCharacterSet]characterIsMember:[value characterAtIndex:0]])return [@"T"stringByAppendingString:value];return value;}
...
- (void)drawTaxiwayLabels:(NSMutableArray *)occupied {
    NSMutableDictionary *groups=[NSMutableDictionary dictionary],*lengths=[NSMutableDictionary dictionary];for(NSDictionary *feature in self.canvas.taxiwayFeatures){NSString *reference=[[[feature objectForKey:@"ref"]?:@"" stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]uppercaseString];if(!reference.length||reference.length>7)continue;CGFloat length=0;NSArray *values=[self screenCandidatesForFeature:feature spacing:112 length:&length];if(!values.count)continue;NSMutableArray *group=[groups objectForKey:reference];if(!group){group=[NSMutableArray array];[groups setObject:group forKey:reference];}[group addObjectsFromArray:values];[lengths setObject:[NSNumber numberWithDouble:[[lengths objectForKey:reference]doubleValue]+length]forKey:reference];}
    NSArray *references=[[groups allKeys]sortedArrayUsingFunction:SkyTaxiReferenceSort context:lengths];UIFont *font=[UIFont boldSystemFontOfSize:11];for(NSString *reference in references){NSMutableArray *accepted=[NSMutableArray array];BOOL drew=NO;for(NSValue *value in [groups objectForKey:reference]){CGPoint point=[value CGPointValue];BOOL duplicate=NO;for(NSValue *priorValue in accepted){CGPoint prior=[priorValue CGPointValue];if(hypot(point.x-prior.x,point.y-prior.y)<78){duplicate=YES;break;}}if(duplicate)continue;NSValue *placed=[self availablePointForText:reference around:point font:font occupied:occupied margin:3];if(!placed&&!drew)placed=[self availablePointForText:reference around:point font:font occupied:occupied margin:0];if(!placed)continue;CGPoint labelPoint=[placed CGPointValue];CGRect box=SkyLabelRect(reference,labelPoint,font,1);SkyDrawLabel(reference,labelPoint,font,[UIColor colorWithRed:1 green:.86 blue:.08 alpha:1],[UIColor colorWithWhite:.03 alpha:.92],[UIColor colorWithWhite:.25 alpha:1],1);[occupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-3,-3)]];[accepted addObject:[NSValue valueWithCGPoint:point]];drew=YES;}}
}
...
@property(nonatomic,assign) BOOL labelRefreshScheduled;
@end

@implementation AirportMapViewController
@synthesize icao=_icao,mapPath=_mapPath,mapData=_mapData,scrollView=_scrollView,canvas=_canvas,labelOverlay=_labelOverlay,attributionLabel=_attributionLabel,lastLayoutSize=_lastLayoutSize,hasAppeared=_hasAppeared,labelRefreshScheduled=_labelRefreshScheduled;
...
- (void)refreshMapLabels { self.labelRefreshScheduled=NO;self.labelOverlay.hidden=NO;[self.labelOverlay setNeedsDisplay]; }
- (void)scheduleMapLabelRefresh { if(self.labelRefreshScheduled)return;self.labelRefreshScheduled=YES;[self performSelector:@selector(refreshMapLabels)withObject:nil afterDelay:.03]; }
- (void)forceMapLabelRefresh { [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(refreshMapLabels)object:nil];self.labelRefreshScheduled=NO;[self refreshMapLabels]; }
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale { [self.canvas configureForDisplayScale:scale];[self forceMapLabelRefresh]; }
...
- (void)dealloc { [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(refreshMapLabels)object:nil];[_icao release];[_mapPath release];[_mapData release];[_scrollView release];[_canvas release];[_labelOverlay release];[_attributionLabel release];[super dealloc]; }
@end
