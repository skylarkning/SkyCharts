...
- (void)drawStandLabels:(NSMutableArray *)occupied {
    CGFloat fit=MAX(.001,self.scrollView.minimumZoomScale),detail=self.scrollView.zoomScale/fit;if(detail<1.65)return;UIFont *font=[UIFont boldSystemFontOfSize:detail>=4?8.5:9.5];CGFloat margin=detail>=4?1:2;CGContextRef context=UIGraphicsGetCurrentContext();for(NSInteger pass=0;pass<2;pass++)for(NSDictionary *feature in self.canvas.standFeatures){NSString *kind=[feature objectForKey:@"kind"];if((pass==0&&![kind isEqual:@"gate"])||(pass==1&&![kind isEqual:@"parking_position"]))continue;NSString *reference=[feature objectForKey:@"ref"]?:[feature objectForKey:@"name"];NSArray *points=[feature objectForKey:@"points"];if(!reference.length||!points.count)continue;CGPoint anchor=[self screenPointForCoordinate:[points lastObject]];if(!CGRectContainsPoint(CGRectInset(self.bounds,-28,-28),anchor))continue;CGPoint preferred=CGPointMake(anchor.x,anchor.y-10);NSValue *placed=[self availablePointForText:reference around:preferred font:font occupied:occupied margin:margin];if(!placed)continue;CGPoint labelPoint=[placed CGPointValue];CGContextSetStrokeColorWithColor(context,[UIColor colorWithWhite:.70 alpha:.72].CGColor);CGContextSetLineWidth(context,.65);CGContextMoveToPoint(context,anchor.x,anchor.y);CGContextAddLineToPoint(context,labelPoint.x,labelPoint.y);CGContextStrokePath(context);CGRect box=SkyLabelRect(reference,labelPoint,font,1);SkyDrawLabel(reference,labelPoint,font,[UIColor colorWithWhite:.94 alpha:1],[UIColor colorWithWhite:.04 alpha:.88],[UIColor colorWithWhite:.58 alpha:.85],1);[occupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-margin,-margin)]];}
}
- (void)drawRect:(CGRect)rect { NSMutableArray *occupied=[NSMutableArray array];[self reserveRunwayBadges:occupied];[self drawTaxiwayLabels:occupied];[self drawStandLabels:occupied]; }
@end
...
@synthesize icao=_icao,mapPath=_mapPath,mapData=_mapData,scrollView=_scrollView,canvas=_canvas,labelOverlay=_labelOverlay,attributionLabel=_attributionLabel,lastLayoutSize=_lastLayoutSize,hasAppeared=_hasAppeared;
...
- (void)refreshMapLabels { self.labelOverlay.hidden=NO;[self.labelOverlay setNeedsDisplay]; }
- (void)scrollViewDidZoom:(UIScrollView *)scrollView { [self centerMap]; }
- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view { self.labelOverlay.hidden=YES; }
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale { [self refreshMapLabels]; }
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView { self.labelOverlay.hidden=YES; }
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate { if(!decelerate)[self refreshMapLabels]; }
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView { [self refreshMapLabels]; }
...
- (void)dealloc { [_icao release];[_mapPath release];[_mapData release];[_scrollView release];[_canvas release];[_labelOverlay release];[_attributionLabel release];[super dealloc]; }
@end
