...
#define SKY_LABEL_GRID_COLUMNS 72
#define SKY_LABEL_GRID_ROWS 64

typedef struct {
    CGFloat width;
    CGFloat height;
    unsigned char cells[SKY_LABEL_GRID_COLUMNS * SKY_LABEL_GRID_ROWS];
} SkyLabelGrid;

static void SkyLabelGridInit(SkyLabelGrid *grid, CGSize size) {
    grid->width=MAX(1,size.width);grid->height=MAX(1,size.height);memset(grid->cells,0,sizeof(grid->cells));
}

static BOOL SkyLabelGridRange(SkyLabelGrid *grid, CGRect rect, NSInteger *left, NSInteger *top, NSInteger *right, NSInteger *bottom) {
    if(!grid)return NO;CGRect bounds=CGRectMake(0,0,grid->width,grid->height),clipped=CGRectIntersection(bounds,rect);if(CGRectIsNull(clipped)||CGRectIsEmpty(clipped))return NO;
    *left=MAX(0,MIN(SKY_LABEL_GRID_COLUMNS-1,(NSInteger)floor(CGRectGetMinX(clipped)/grid->width*SKY_LABEL_GRID_COLUMNS)));
    *right=MAX(0,MIN(SKY_LABEL_GRID_COLUMNS-1,(NSInteger)floor((CGRectGetMaxX(clipped)-.01)/grid->width*SKY_LABEL_GRID_COLUMNS)));
    *top=MAX(0,MIN(SKY_LABEL_GRID_ROWS-1,(NSInteger)floor(CGRectGetMinY(clipped)/grid->height*SKY_LABEL_GRID_ROWS)));
    *bottom=MAX(0,MIN(SKY_LABEL_GRID_ROWS-1,(NSInteger)floor((CGRectGetMaxY(clipped)-.01)/grid->height*SKY_LABEL_GRID_ROWS)));
    return YES;
}

static BOOL SkyLabelGridFits(SkyLabelGrid *grid, CGRect box, CGFloat margin) {
    if(!grid)return YES;NSInteger left,top,right,bottom;if(!SkyLabelGridRange(grid,CGRectInset(box,-margin,-margin),&left,&top,&right,&bottom))return YES;
    for(NSInteger row=top;row<=bottom;row++)for(NSInteger column=left;column<=right;column++)if(grid->cells[row*SKY_LABEL_GRID_COLUMNS+column])return NO;return YES;
}

static void SkyLabelGridOccupy(SkyLabelGrid *grid, CGRect box, CGFloat margin) {
    if(!grid)return;NSInteger left,top,right,bottom;if(!SkyLabelGridRange(grid,CGRectInset(box,-margin,-margin),&left,&top,&right,&bottom))return;
    for(NSInteger row=top;row<=bottom;row++)for(NSInteger column=left;column<=right;column++)grid->cells[row*SKY_LABEL_GRID_COLUMNS+column]=1;
}
...
static CGFloat SkyRunwayDesignatorHeading(NSString *designator) {
    NSInteger number=0;NSUInteger digits=0;for(NSUInteger index=0;index<designator.length;index++){unichar character=[designator characterAtIndex:index];if(character<'0'||character>'9')break;number=number*10+(character-'0');digits++;}if(!digits||number<1||number>36)return NAN;return fmod(number*10.0,360.0);
}

static CGFloat SkyHeadingDifference(CGFloat left, CGFloat right) {
    CGFloat difference=fabs(left-right);while(difference>=360)difference-=360;return MIN(difference,360-difference);
}

static void SkyOrientRunwayEndpoints(NSString *reference, CGPoint *start, CGPoint *end) {
    NSArray *ends=[reference componentsSeparatedByString:@"/"];if(ends.count<2)return;CGFloat expected=SkyRunwayDesignatorHeading([ends objectAtIndex:0]);if(isnan(expected))return;CGFloat dx=end->x-start->x,dy=end->y-start->y,forward=atan2(dx,-dy)*180.0/M_PI;if(forward<0)forward+=360;CGFloat reverse=fmod(forward+180,360);if(SkyHeadingDifference(reverse,expected)<SkyHeadingDifference(forward,expected)){CGPoint swap=*start;*start=*end;*end=swap;}
}
...
- (id)initWithMapData:(NSDictionary *)mapData size:(CGSize)size {
    if((self=[super initWithFrame:CGRectMake(0,0,size.width,size.height)])){
        ...
        NSMutableArray *aprons=[NSMutableArray array],*hangars=[NSMutableArray array],*taxiways=[NSMutableArray array],*stands=[NSMutableArray array],*terminals=[NSMutableArray array];NSMutableDictionary *runwayGroups=[NSMutableDictionary dictionary];NSUInteger anonymous=0;
        for(NSDictionary *feature in self.features){NSString *kind=[feature objectForKey:@"kind"];
            if([kind isEqual:@"apron"])[aprons addObject:feature];
            else if([kind isEqual:@"hangar"])[hangars addObject:feature];
            else if([kind isEqual:@"terminal"])[terminals addObject:feature];
...
- (void)drawRect:(CGRect)rect {
    ...
    for(NSDictionary *feature in self.backgroundFeatures){NSString *kind=[feature objectForKey:@"kind"],*operator=[feature objectForKey:@"operator"]?:@"";UIColor *fill=[self fillForKind:kind];UIBezierPath *path=[self pathForFeature:feature];if(!fill||!path)continue;[fill setFill];[path fill];BOOL gatewayTerminal=[kind isEqual:@"terminal"]&&[operator rangeOfString:@"X-Plane Scenery Gateway"].location!=NSNotFound;if(!gatewayTerminal){[[UIColor colorWithWhite:.48 alpha:.55]setStroke];path.lineWidth=1.5/MAX(.01,self.labelScale);[path stroke];}}
...
- (void)updateTaxiCenterlineForDisplayScale:(CGFloat)displayScale { if(displayScale<=0)return;CGFloat width=.78/MAX(.01,displayScale),current=self.taxiCenterlineLayer.lineWidth;if(current>0&&fabs(current-width)/MAX(.01,width)<.055)return;[CATransaction begin];[CATransaction setDisableActions:YES];self.taxiCenterlineLayer.lineWidth=width;[CATransaction commit]; }
...
@property(nonatomic,retain) NSDictionary *taxiRecordGroups;
@property(nonatomic,retain) NSArray *taxiReferenceOrder;
...
@synthesize canvas=_canvas,scrollView=_scrollView,taxiLabelRecords=_taxiLabelRecords,taxiRecordGroups=_taxiRecordGroups,taxiReferenceOrder=_taxiReferenceOrder,standLabelRecords=_standLabelRecords,terminalLabelRecords=_terminalLabelRecords,runwayLabelRecords=_runwayLabelRecords,canvasScreenOrigin=_canvasScreenOrigin,canvasScreenScale=_canvasScreenScale,visibleCanvasRect=_visibleCanvasRect;
...
- (void)prepareLabelRecords {
    ...
    NSMutableDictionary *taxiGroups=[NSMutableDictionary dictionary],*taxiLengths=[NSMutableDictionary dictionary];for(NSDictionary *record in taxiRecords){NSString *reference=[record objectForKey:@"ref"];NSMutableArray *group=[taxiGroups objectForKey:reference];if(!group){group=[NSMutableArray array];[taxiGroups setObject:group forKey:reference];}[group addObject:record];[taxiLengths setObject:[NSNumber numberWithDouble:[[taxiLengths objectForKey:reference]doubleValue]+[[record objectForKey:@"length"]doubleValue]]forKey:reference];}
    self.taxiLabelRecords=taxiRecords;self.taxiRecordGroups=taxiGroups;self.taxiReferenceOrder=[[taxiGroups allKeys]sortedArrayUsingFunction:SkyTaxiReferenceSort context:taxiLengths];self.standLabelRecords=standRecords;self.terminalLabelRecords=terminalRecords;self.runwayLabelRecords=runwayRecords;
}
...
- (NSValue *)availablePointForText:(NSString *)text around:(CGPoint)point font:(UIFont *)font grid:(SkyLabelGrid *)grid margin:(CGFloat)margin {
    static const CGFloat offsets[][2]={{0,0},{0,-14},{0,14},{-18,0},{18,0},{-14,-13},{14,-13},{-14,13},{14,13}};CGRect safe=CGRectInset(self.bounds,2,2);for(NSUInteger index=0;index<9;index++){CGPoint candidate=CGPointMake(point.x+offsets[index][0],point.y+offsets[index][1]);CGRect box=SkyLabelRect(text,candidate,font,1);if(CGRectContainsRect(safe,box)&&SkyLabelGridFits(grid,box,margin))return [NSValue valueWithCGPoint:candidate];}return nil;
}
...
- (void)drawRunwayLabels:(SkyLabelGrid *)grid {
    UIFont *font=[UIFont boldSystemFontOfSize:15];for(NSDictionary *record in self.runwayLabelRecords){CGPoint anchor=[self screenPointForCanvasValue:[record objectForKey:@"point"]];if(!CGRectContainsPoint(CGRectInset(self.bounds,-40,-40),anchor))continue;NSString *text=[record objectForKey:@"text"];NSValue *placed=[self availablePointForText:text around:anchor font:font grid:grid margin:5];if(!placed)continue;CGPoint point=[placed CGPointValue];CGRect box=SkyLabelRect(text,point,font,1);SkyDrawLabel(text,point,font,[UIColor whiteColor],[UIColor colorWithWhite:.02 alpha:.94],[UIColor colorWithWhite:.76 alpha:1],1);SkyLabelGridOccupy(grid,box,5);}
}
- (void)drawTaxiwayLabels:(SkyLabelGrid *)grid {
    CGFloat fit=MAX(.001,self.scrollView.minimumZoomScale),detail=MAX(1,self.scrollView.zoomScale/fit);UIFont *font=[UIFont boldSystemFontOfSize:11];NSUInteger density=(NSUInteger)MAX(22,MIN(50,self.bounds.size.width*self.bounds.size.height/17000.0)),lodBonus=(NSUInteger)MIN(12,MAX(0,log(detail)/log(2.0)*6)),labelLimit=MIN(60,density+lodBonus),labelsDrawn=0;CGContextRef context=UIGraphicsGetCurrentContext();
    for(NSString *reference in self.taxiReferenceOrder){NSMutableArray *values=[NSMutableArray array];CGFloat screenLength=0;for(NSDictionary *record in [self.taxiRecordGroups objectForKey:reference]){CGFloat length=0;NSArray *recordValues=[self screenCandidatesForRecord:record spacing:145 length:&length];if(recordValues.count)[values addObjectsFromArray:recordValues];screenLength+=length;}if(!values.count)continue;CGFloat baseLength=screenLength/detail,reveal=baseLength>=85?.7:(baseLength>=45?1.2:(baseLength>=24?1.8:2.7)),alpha=MIN(1,MAX(0,(detail-(reveal-.45))/.45));if(alpha<=.03)continue;NSMutableArray *accepted=[NSMutableArray array];BOOL drew=NO;NSUInteger referenceLimit=MIN(4,MAX(1,(NSUInteger)ceil(screenLength/360.0))),referenceCount=0;CGContextSaveGState(context);CGContextSetAlpha(context,alpha);for(NSValue *value in values){if(labelsDrawn>=labelLimit||referenceCount>=referenceLimit)break;CGPoint point=[value CGPointValue];BOOL duplicate=NO;for(NSValue *priorValue in accepted){CGPoint prior=[priorValue CGPointValue];if(hypot(point.x-prior.x,point.y-prior.y)<110){duplicate=YES;break;}}if(duplicate)continue;NSValue *placed=[self availablePointForText:reference around:point font:font grid:grid margin:4];if(!placed&&!drew)placed=[self availablePointForText:reference around:point font:font grid:grid margin:1];if(!placed)continue;CGPoint labelPoint=[placed CGPointValue];CGRect box=SkyLabelRect(reference,labelPoint,font,1);SkyDrawLabel(reference,labelPoint,font,[UIColor colorWithRed:1 green:.86 blue:.08 alpha:1],[UIColor colorWithWhite:.03 alpha:.92],[UIColor colorWithWhite:.25 alpha:1],1);SkyLabelGridOccupy(grid,box,4);[accepted addObject:[NSValue valueWithCGPoint:point]];drew=YES;labelsDrawn++;referenceCount++;}CGContextRestoreGState(context);if(labelsDrawn>=labelLimit)break;}
}
- (void)drawTerminalLabels:(SkyLabelGrid *)grid {
    UIFont *font=[UIFont boldSystemFontOfSize:10];CGContextRef context=UIGraphicsGetCurrentContext();NSMutableDictionary *accepted=[NSMutableDictionary dictionary];for(NSDictionary *record in self.terminalLabelRecords){NSString *reference=[record objectForKey:@"ref"];CGPoint anchor=[self screenPointForCanvasValue:[record objectForKey:@"point"]];if(!CGRectContainsPoint(CGRectInset(self.bounds,-35,-35),anchor))continue;NSValue *previous=[accepted objectForKey:reference];if(previous&&hypot(anchor.x-[previous CGPointValue].x,anchor.y-[previous CGPointValue].y)<90)continue;NSValue *placed=[self availablePointForText:reference around:anchor font:font grid:grid margin:2];if(!placed)placed=[self availablePointForText:reference around:anchor font:font grid:NULL margin:0];if(!placed)continue;CGPoint labelPoint=[placed CGPointValue];CGContextSetStrokeColorWithColor(context,[UIColor colorWithWhite:.70 alpha:.72].CGColor);CGContextSetLineWidth(context,.65);CGContextMoveToPoint(context,anchor.x,anchor.y);CGContextAddLineToPoint(context,labelPoint.x,labelPoint.y);CGContextStrokePath(context);CGRect box=SkyLabelRect(reference,labelPoint,font,1);SkyDrawLabel(reference,labelPoint,font,[UIColor colorWithWhite:.94 alpha:1],[UIColor colorWithWhite:.04 alpha:.88],[UIColor colorWithWhite:.58 alpha:.85],1);SkyLabelGridOccupy(grid,box,2);[accepted setObject:[NSValue valueWithCGPoint:anchor]forKey:reference];}
}
- (void)drawStandLabels:(SkyLabelGrid *)grid {
    CGFloat fit=MAX(.001,self.scrollView.minimumZoomScale),detail=self.scrollView.zoomScale/fit,alpha=MIN(1,MAX(0,(detail-1.2)/.65));if(alpha<=.01)return;CGFloat fontSize=MAX(8.5,9.7-MIN(1.2,MAX(0,detail-1.65)*.45));UIFont *font=[UIFont boldSystemFontOfSize:fontSize];CGFloat margin=MAX(1,2.2-MIN(1.2,MAX(0,detail-1.65)*.35));NSUInteger density=(NSUInteger)MAX(60,MIN(125,self.bounds.size.width*self.bounds.size.height/7000.0)),lodBonus=(NSUInteger)MIN(25,MAX(0,(detail-1.5)*10)),labelLimit=MIN(150,density+lodBonus),labelsDrawn=0;CGContextRef context=UIGraphicsGetCurrentContext();CGContextSaveGState(context);CGContextSetAlpha(context,alpha);for(NSInteger pass=0;pass<2&&labelsDrawn<labelLimit;pass++)for(NSDictionary *record in self.standLabelRecords){if(labelsDrawn>=labelLimit)break;NSString *kind=[record objectForKey:@"kind"];if((pass==0&&![kind isEqual:@"gate"])||(pass==1&&![kind isEqual:@"parking_position"]))continue;NSString *reference=[record objectForKey:@"ref"];CGPoint anchor=[self screenPointForCanvasValue:[record objectForKey:@"point"]];if(!CGRectContainsPoint(CGRectInset(self.bounds,-28,-28),anchor))continue;CGPoint preferred=CGPointMake(anchor.x,anchor.y-10);NSValue *placed=[self availablePointForText:reference around:preferred font:font grid:grid margin:margin];if(!placed)continue;CGPoint labelPoint=[placed CGPointValue];CGContextSetStrokeColorWithColor(context,[UIColor colorWithWhite:.70 alpha:.72].CGColor);CGContextSetLineWidth(context,.65);CGContextMoveToPoint(context,anchor.x,anchor.y);CGContextAddLineToPoint(context,labelPoint.x,labelPoint.y);CGContextStrokePath(context);CGRect box=SkyLabelRect(reference,labelPoint,font,1);SkyDrawLabel(reference,labelPoint,font,[UIColor colorWithWhite:.94 alpha:1],[UIColor colorWithWhite:.04 alpha:.88],[UIColor colorWithWhite:.58 alpha:.85],1);SkyLabelGridOccupy(grid,box,margin);labelsDrawn++;}CGContextRestoreGState(context);
}
- (void)drawRect:(CGRect)rect { [self updateCanvasTransform];SkyLabelGrid grid;SkyLabelGridInit(&grid,self.bounds.size);[self drawRunwayLabels:&grid];[self drawTerminalLabels:&grid];[self drawTaxiwayLabels:&grid];[self drawStandLabels:&grid]; }
- (void)dealloc { [_taxiLabelRecords release];[_taxiRecordGroups release];[_taxiReferenceOrder release];[_standLabelRecords release];[_terminalLabelRecords release];[_runwayLabelRecords release];[super dealloc]; }
@end
