...
- (void)drawRect:(CGRect)rect {
    ...
    for(NSDictionary *feature in self.taxiwayFeatures){UIBezierPath *path=[self pathForFeature:feature];if(!path)continue;NSString *kind=[feature objectForKey:@"kind"];path.lineCapStyle=kCGLineCapRound;path.lineJoinStyle=kCGLineJoinRound;[[UIColor colorWithWhite:.40 alpha:1]setStroke];path.lineWidth=([kind isEqual:@"taxilane"]?5:9)/MAX(.01,self.labelScale);[path stroke];}
    for(NSDictionary *feature in self.taxiwayFeatures){UIBezierPath *path=[self pathForFeature:feature];if(!path)continue;path.lineCapStyle=kCGLineCapRound;path.lineJoinStyle=kCGLineJoinRound;[[UIColor colorWithRed:1 green:.79 blue:.03 alpha:1]setStroke];path.lineWidth=1.2/MAX(.01,self.labelScale);[path stroke];}
    ...
    NSMutableArray *standOccupied=[NSMutableArray arrayWithArray:occupied];UIFont *standFont=[UIFont boldSystemFontOfSize:8.5/standScale];for(NSInteger pass=0;pass<2;pass++)for(NSDictionary *feature in self.standFeatures){NSString *kind=[feature objectForKey:@"kind"];if((pass==0&&![kind isEqual:@"gate"])||(pass==1&&![kind isEqual:@"parking_position"]))continue;NSString *reference=[[feature objectForKey:@"ref"]?:[feature objectForKey:@"name"] description];NSArray *points=[feature objectForKey:@"points"];if(!reference.length||!points.count)continue;CGPoint anchor=[self mapPoint:[points lastObject]],point=CGPointMake(anchor.x,anchor.y-7/standScale);CGRect box=SkyLabelRect(reference,point,standFont,standScale);if(!SkyLabelFits(box,standOccupied,5,standScale))continue;CGContextSetStrokeColorWithColor(context,[UIColor colorWithWhite:.72 alpha:.7].CGColor);CGContextSetLineWidth(context,.6/standScale);CGContextMoveToPoint(context,anchor.x,anchor.y);CGContextAddLineToPoint(context,point.x,point.y);CGContextStrokePath(context);SkyDrawLabel(reference,point,standFont,[UIColor colorWithWhite:.92 alpha:1],[UIColor colorWithWhite:.06 alpha:.78],[UIColor colorWithWhite:.55 alpha:.8],standScale);[standOccupied addObject:[NSValue valueWithCGRect:CGRectInset(box,-5/standScale,-5/standScale)]];}
